#!/usr/bin/env bash
set -euo pipefail
# เปลี่ยนชื่อแอป/บริการได้ที่ APP_SLUG จุดเดียว
PROJECT_NAME="RedHub-XBot"
APP_SLUG="xbot"
INSTALL_DIR_NAME="${APP_SLUG}"
DEFAULT_SERVICE_NAME="${APP_SLUG}"
...
ROOT_DIR="${HOME_DIR}/${INSTALL_DIR_NAME}"
...
if [[ "$SOURCE_HAS_GIT" != "1" && ! -d ".git" ]]; then
  ...
  git add -A
  GIT_AUTHOR_NAME="${APP_SLUG} installer" \
  GIT_AUTHOR_EMAIL="installer@localhost" \
  GIT_COMMITTER_NAME="${APP_SLUG} installer" \
