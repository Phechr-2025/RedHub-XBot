...
if prompt_yes_no "ตั้งค่าไฟล์ .env เลยไหม / [Y/N] (พิมพ์ใหญ่พิมพ์เล็กก็ได้)"; then
  ...
  WEB_HTTP_PORT="$(prompt_env "พอร์ตเว็บไซต์ HTTP" "9090")"
  WEB_HTTPS_PORT="$(prompt_env "พอร์ตเว็บไซต์ HTTPS" "9943")"
else
...
WEB_PORT=$(escape_env_value "$WEB_HTTP_PORT")
