...
WEB_PORT="$(prompt_env "พอร์ตเว็บไซต์ภายใน" "2026")"
WEB_HTTP_PORT="$(prompt_env "พอร์ต HTTP" "9090")"
WEB_HTTPS_PORT="$(prompt_env "พอร์ต HTTPS" "9943")"
...
WEB_HTTP_PORT=$(escape_env_value "$WEB_HTTP_PORT")
WEB_HTTPS_PORT=$(escape_env_value "$WEB_HTTPS_PORT")
...
if [[ -x "${ROOT_DIR}/webctl.sh" ]]; then
  info "ตั้งค่าเว็บให้ใช้งานผ่านพอร์ต HTTP ${WEB_HTTP_PORT}"
  bash "${ROOT_DIR}/webctl.sh" set-http-port "${WEB_HTTP_PORT}" || warn "ตั้งค่า HTTP proxy อัตโนมัติไม่สำเร็จ"
fi
...
if [[ -n "${WEB_DOMAIN}" ]]; then
  ...
  bash "${ROOT_DIR}/webctl.sh" add-domain "${WEB_DOMAIN}" || warn "ผูกโดเมนอัตโนมัติไม่สำเร็จ กรุณาใช้เมนูจัดการเว็บไซต์ภายหลัง"
fi
...
info "Web URL: http://$(hostname -I 2>/dev/null | awk '{print $1}'):$(grep -E '^WEB_HTTP_PORT=' .env | cut -d= -f2 | tr -d '"')$(grep -E '^WEB_PANEL_PATH=' .env | cut -d= -f2 | tr -d '"')"
