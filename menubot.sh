...
get_latest_release_source() {
  local release_output release_lines latest_tag latest_url release_work_dir release_source_dir
  ...
  if [[ -z "$release_work_dir" || -z "$release_source_dir" || ! -d "$release_source_dir" ]]; then
    cleanup_path "$release_work_dir"
    return 1
  fi

  printf '%s\n%s\n%s\n' "$latest_tag" "$release_work_dir" "$release_source_dir"
}
...
update_menu_only() {
  ...
  local release_output release_lines latest_tag release_work_dir release_source_dir
  ...
  if [[ -z "$latest_tag" || -z "$release_source_dir" || ! -d "$release_source_dir" ]]; then
    err "อ่านข้อมูล Release สำหรับเมนูไม่สำเร็จ"
    cleanup_path "$release_work_dir"
    exit 1
  fi

  info "อัปเดต menubot.sh และ webctl.sh จาก Release: ${latest_tag}"
  install -m 755 "$release_source_dir/menubot.sh" "$PROJECT_DIR/menubot.sh"
  install -m 755 "$release_source_dir/webctl.sh" "$PROJECT_DIR/webctl.sh" 2>/dev/null || true
...
update_specific_release() {
  ...
  local target="${1:-}"
  if [[ -z "$target" ]]; then
    err "ต้องระบุ tag หรือ URL"
    exit 1
  fi

  local tag="$target"
  if [[ "$target" =~ github\.com/.*/releases/tag/([^/?#]+) ]]; then
    tag="${BASH_REMATCH[1]}"
  fi

  local release_output release_lines latest_tag latest_url release_work_dir release_source_dir
  if release_output="$(python3 - "$REPO_OWNER" "$REPO_NAME" "$tag" <<'PY'
...
with urllib.request.urlopen(req, timeout=30) as resp:
    data = json.load(resp)
release_tag = str(data.get("tag_name", "")).strip()
tarball_url = str(data.get("tarball_url", "")).strip()
if not release_tag or not tarball_url:
    raise SystemExit("release metadata missing")
print(release_tag)
print(tarball_url)
PY
    ...
    err "ไม่สามารถอ่าน Release ตาม tag ที่ระบุได้"
    exit 1
  fi

  if [[ -z "$latest_tag" || -z "$latest_url" ]]; then
    err "ข้อมูล Release ไม่ครบ"
    exit 1
  fi
  ...
  chmod +x "$PROJECT_DIR/webctl.sh" "$PROJECT_DIR/web_panel.py" 2>/dev/null || true
  ensure_virtualenv
  printf '%s\n' "$latest_tag" > "$VERSION_FILE"
  cleanup_path "$release_work_dir"
...
web_set_http_port() {
  local port=""
  read -r -p "พอร์ต HTTP ใหม่ (1-65535): " port
  webctl set-http-port "$port"
}

web_set_https_port() {
  local port=""
  read -r -p "พอร์ต HTTPS ใหม่ (1-65535): " port
...
7) เริ่มเว็บ
8) รีสตาร์ทเว็บ
9) เพิ่ม/เปลี่ยนโดเมน
10) อัปเดตไลบารี่เว็บ [เฉพาะที่เว็บไซต์ใช้]
11) เช็คสถานะเว็บ [เฉพาะสถานะของเว็บไซต์]
0) กลับ
EOF_WEB
...
3) รีสตาร์ทระบบบอท
4) จัดการเว็บไซต์
5) อัปเดตทุกอย่าง (ยกเว้นเมนู)
6) อัปเดตเมนู
7) อัปเดตแบบเจาะจง (tag หรือ GitHub release URL)
...
EOF_MENU
  ...
  case "$choice" in
    ...
    6) update_menu_only ;;
    7)
      read -r -p "วาง tag หรือ GitHub release URL: " target
      update_specific_release "$target"
      ;;
