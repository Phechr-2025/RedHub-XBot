...
uninstall_app() {
  ...
  legacy_dirs=()
  for candidate in "$HOME_BASE"/*shop3x* "$HOME_BASE"/*Shop3x*; do
...
update_script() {
  ...
  local current_version latest_tag latest_url release_source_dir
  ...
  if ! release_source_dir="$(download_release_source "$latest_url" 2>/dev/null)"; then
