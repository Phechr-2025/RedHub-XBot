...
- เปิดด้วย URL ตาม `WEB_HTTP_PORT` และ path ที่สุ่มไว้ใน `WEB_PANEL_PATH`
- ถ้าเปิดแค่ `http://IP:WEB_HTTP_PORT` ระบบจะพาไปยัง path ที่สุ่มไว้ให้อัตโนมัติ
- ถ้าผูกโดเมนและมีใบรับรองแล้ว จะเข้า `https://โดเมน:WEB_HTTPS_PORT` ได้ด้วย
- แก้ค่า `.env` ผ่านหน้าเว็บได้เลย

- หน้าตา Web Panel ทำเป็น sidebar คล้าย 3x-ui รุ่นใหม่ มีปุ่มสามขีดซ้ายบน และเมนู 3 รายการคือ `ภาพรวม` `สถานะบอท` และ `ตั้งค่าทั่วไป`
- เมนูจัดการเว็บไซต์มีคำสั่งแยกชัดเจนสำหรับดู URL, สถานะ, รีสตาร์ท, เปลี่ยน path, ตั้งค่า HTTP/HTTPS port, เพิ่มโดเมน และอัปเดตไลบารี่เว็บ

- ถ้าผูกโดเมนและ DNS ชี้มาที่ VPS แล้ว ระบบจะขอใบรับรอง Let's Encrypt ให้ และเว็บพาเนลจะเปิด HTTPS ตามพอร์ตที่ตั้งไว้
