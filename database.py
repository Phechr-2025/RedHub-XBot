...
def list_sale_categories() -> List[Dict]:
    conn = _get_conn()
    rows = conn.execute(
        "SELECT * FROM sale_categories WHERE active = 1 ORDER BY sort_order ASC, id ASC"
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def create_sale_category(name: str, description: str = "", sort_order: int = 0) -> int:
    conn = _get_conn()
    try:
        cur = conn.execute(
            "INSERT INTO sale_categories (name, description, sort_order, active) VALUES (?, ?, ?, 1)",
...
def update_sale_category(category_id: int, name: str, description: str = "", sort_order: int = 0, active: int = 1) -> None:
    conn = _get_conn()
    conn.execute(
        """
        ...
        WHERE id = ?
        """,
        (name.strip(), description.strip(), int(sort_order), int(active), int(category_id)),
    )
    conn.commit()
    conn.close()
