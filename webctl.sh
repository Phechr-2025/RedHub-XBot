...
server {
    ...
    location ${path} {
        proxy_pass http://127.0.0.1:${internal_port}${path};
        proxy_http_version 1.1;
        proxy_set_header Host \$host;
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
...
show_urls() {
  refresh_env
  local ip http_url https_url domain service_state
  ip="$(public_ip)"
  domain="${WEB_DOMAIN:-}"
...
check_web_status() {
  ...
  echo "HTTPS firewall : $(port_allowed_now "${WEB_HTTPS_PORT}" && echo เปิดแล้ว || echo ยังไม่ยืนยัน)"
  echo "=============================="
