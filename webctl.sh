...
refresh_env() {
  ...
  WEB_HTTP_PORT="${WEB_HTTP_PORT:-9090}"
...
port_allowed_now() {
  ...
  if command -v ufw >/dev/null 2>&1; then
    ...
    if grep -Eq "(${port}/tcp|${port}\b)" <<<"$status"; then
      return 0
    fi
  fi
  if command -v firewall-cmd >/dev/null 2>&1; then
...
ensure_port_open_or_warn() {
  local port="$1"
  local label="$2"
  if port_open_via_firewall "$port"; then
    if port_allowed_now "$port"; then
      info "เปิดพอร์ต ${label} (${port}) เรียบร้อย"
      return 0
    fi
    warn "สั่งเปิดพอร์ต ${label} (${port}) แล้ว แต่ยังตรวจสอบสถานะไม่ยืนยัน"
    return 0
  fi
  warn "เปิดพอร์ต ${label} (${port}) ด้วยคำสั่งไม่ได้"
  warn "กรุณาเปิดพอร์ตนี้ในตั้งค่า VPS / Security Group / Firewall ของผู้ให้บริการ"
  return 1
}
...
write_nginx_conf() {
  ...
  if [[ -n "$domain" && -d "$cert_dir" && -f "${cert_dir}/fullchain.pem" && -f "${cert_dir}/privkey.pem" ]]; then
    cat > "$NGINX_CONF" <<EOF
server {
...
remove_http_proxy() {
  rm -f "$NGINX_LINK" "$NGINX_CONF" >/dev/null 2>&1 || true
...
apply_http_proxy() {
  ...
  ensure_port_open_or_warn "$http_port" "HTTP" || true

  # โหมดไม่มีโดเมน: ให้เว็บฟังพอร์ต HTTP ตรง ๆ จึงไม่ต้องใช้ nginx
  if [[ -z "$domain" ]]; then
    remove_http_proxy
    return 0
  fi
  ...
  ensure_port_open_or_warn "${https_port:-9443}" "HTTPS" || true
  write_nginx_conf "$domain" "$internal_port" "$path" "$http_port" "${https_port:-0}"
...
ensure_domain_https() {
  ...
  ensure_port_open_or_warn "$http_port" "HTTP" || true
  ensure_port_open_or_warn "$https_port" "HTTPS" || true
  ...
  if [[ ! -d "/etc/letsencrypt/live/${domain}" || ! -f "/etc/letsencrypt/live/${domain}/fullchain.pem" || ! -f "/etc/letsencrypt/live/${domain}/privkey.pem" ]]; then
    ...
    certbot certonly       --standalone       --preferred-challenges http       --http-01-port 80       --non-interactive       --agree-tos       --register-unsafely-without-email       -d "$domain"
  else
    info "พบใบรับรองเดิม ใช้ต่อได้เลย"
  fi

  write_nginx_conf "$domain" "$internal_port" "$path" "$http_port" "$https_port"
...
show_urls() {
  refresh_env
  local ip http_url https_url domain service_state listen_port mode
  ip="$(public_ip)"
  domain="${WEB_DOMAIN:-}"
  http_url="http://${ip}:${WEB_HTTP_PORT}${panel_path}"
  https_url=""
  if [[ -n "$domain" ]]; then
...
check_web_status() {
  ...
  echo "Domain        : ${domain:-ยังไม่ได้ตั้งค่า}"
  echo "HTTP firewall  : $(port_allowed_now "${WEB_HTTP_PORT}" && echo เปิดแล้ว || echo ยังไม่ยืนยัน)"
  echo "HTTPS firewall : $(if [[ -n "$domain" ]]; then port_allowed_now "${WEB_HTTPS_PORT}" && echo เปิดแล้ว || echo ยังไม่ยืนยัน; else echo ยังไม่เกี่ยว; fi)"
  echo "=============================="
...
reset_panel_web() {
  ...
  if [[ -n "${WEB_DOMAIN:-}" ]]; then
    apply_http_proxy "${WEB_DOMAIN}" "${WEB_PORT}" "${panel_path}" "${WEB_HTTP_PORT}" "${WEB_HTTPS_PORT}"
  else
    apply_http_proxy "" "${WEB_PORT}" "${panel_path}" "${WEB_HTTP_PORT}" ""
  fi
...
set_web_port() {
  ...
  restart_web
  info "ตั้งพอร์ตภายในเว็บเป็น ${new_port} แล้ว"
}
...
set_http_port() {
  ...
  ensure_port_open_or_warn "$new_port" "HTTP" || true
  ...
  restart_web
  info "ตั้งพอร์ต HTTP เป็น ${new_port} แล้ว"
}
...
set_https_port() {
  ...
  ensure_port_open_or_warn "$new_port" "HTTPS" || true
  ...
  restart_web
  info "ตั้งพอร์ต HTTPS เป็น ${new_port} แล้ว"
}
...
add_domain() {
  ...
  refresh_env
  if ensure_domain_https "$domain" "${WEB_PORT}" "${panel_path}" "${WEB_HTTP_PORT}" "${WEB_HTTPS_PORT}"; then
