...
ensure_domain_https() {
  ...
  ensure_port_open_or_warn "$http_port" "HTTP"
  ensure_port_open_or_warn "$https_port" "HTTPS"
...
reset_panel_web() {
  ...
  if [[ -n "${WEB_DOMAIN:-}" ]]; then
    write_nginx_conf "${WEB_DOMAIN}" "${WEB_PORT}" "${panel_path}" "${WEB_HTTP_PORT}" "${WEB_HTTPS_PORT}"
...
set_http_port() {
  ...
  ensure_port_open_or_warn "$new_port" "HTTP"
...
set_https_port() {
  ...
  ensure_port_open_or_warn "$new_port" "HTTPS"
