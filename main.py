...
APP_SLUG = os.getenv("APP_SLUG", "").strip() or Path(__file__).resolve().parent.name
SERVICE_NAME = os.getenv("SERVICE_NAME", "").strip() or APP_SLUG
WEB_SERVICE_NAME = os.getenv("WEB_SERVICE_NAME", "").strip() or f"{SERVICE_NAME}-web"
MENU_COMMAND = os.getenv("MENU_COMMAND", "menubot").strip() or "menubot"
APP_DIR = Path(__file__).resolve().parent
WEBCTL_SCRIPT = APP_DIR / "webctl.sh"
VERSION_FILE = Path(os.getenv("VERSION_FILE", str(APP_DIR / ".version")))
REPO_OWNER = "Phechr-2025"
REPO_NAME = "RedHub-XBot"
GITHUB_RELEASE_LATEST_URL = f"https://api.github.com/repos/{REPO_OWNER}/{REPO_NAME}/releases/latest"
ADMIN_IDS = {
    int(x.strip())
...
def get_latest_release_info() -> tuple[str, str]:
    data = _http_json(GITHUB_RELEASE_LATEST_URL)
...
def _download_release_tree(tarball_url: str) -> tuple[Path, Path]:
    work_dir = Path(tempfile.mkdtemp(prefix="redhub-release-"))
    archive_path = work_dir / "release.tar.gz"
    extract_dir = work_dir / "extract"
    extract_dir.mkdir(parents=True, exist_ok=True)

    request = urllib.request.Request(tarball_url, headers={"User-Agent": "Mozilla/5.0"})
    with urllib.request.urlopen(request, timeout=120) as response, open(archive_path, "wb") as out:
    ...
    if not roots:
        raise RuntimeError("Extracted release root not found")
    return work_dir, roots[0]


def _sync_tree_preserving(src: Path, dst: Path, preserve_root_names: tuple[str, ...] = (".env", ".version", ".venv")) -> None:
    src = Path(src)
    dst = Path(dst)
    dst.mkdir(parents=True, exist_ok=True)

    preserve = set(preserve_root_names)
...
def _apply_latest_release_update() -> tuple[str, str]:
    current_version = read_app_version()
    latest_tag, tarball_url = get_latest_release_info()
    if current_version != latest_tag:
        work_dir, release_root = _download_release_tree(tarball_url)
        try:
            _sync_tree_preserving(release_root, APP_DIR)
            VERSION_FILE.write_text(f"{latest_tag}\n", encoding="utf-8")
        finally:
            shutil.rmtree(work_dir, ignore_errors=True)
    return current_version, latest_tag


async def _ensure_virtualenv() -> tuple[int, str, str]:
    venv_python = APP_DIR / ".venv" / "bin" / "python"
    venv_pip = APP_DIR / ".venv" / "bin" / "pip"

    if not venv_python.exists() or not venv_pip.exists():
        await run_shell_command("rm -rf .venv && python3 -m venv .venv", cwd=APP_DIR, timeout=180)

    await run_shell_command(".venv/bin/pip install --upgrade pip", cwd=APP_DIR, timeout=180)
    return await run_shell_command(".venv/bin/pip install -r requirements.txt", cwd=APP_DIR, timeout=300)
...
# ───────────────────────────── sale form / shop helpers ─────────────────────────────

SALE_DISPATCH_INTERVAL_SECONDS = 5
sale_dispatch_started = False
...
class SaleCardView(discord.ui.View):
    def __init__(self, product: Dict, timeout: int = 3600):
        super().__init__(timeout=timeout)
        self.product = product

    @discord.ui.button(label="ซื้อสินค้า", style=discord.ButtonStyle.success, emoji="🛒")
    async def buy_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        dm_text = (
            f"✅ คำสั่งซื้อของคุณ\n"
            f"สินค้า: {product.get('name', '')}\n"
            f"ราคา: {price:.2f} บาท\n"
            f"หมวดหมู่: {product.get('category_name', '')}\n"
            f"รายละเอียดสินค้า:\n{product.get('detail', '')}\n\n"
            f"รายละเอียดฟอร์ม:\n{product.get('form_detail', '')}\n\n"
            f"URL ฟอร์ม: {product.get('form_url', '')}\n"
            f"URL 3x-ui: {product.get('xui_url', '')}\n"
            f"Inbound ID: {product.get('inbound_id', '')}\n"
            f"ช่องทาง: {product.get('channel_id', '')}"
        )
        try:
            await user.send(dm_text)
            purchase_id = await asyncio.to_thread(
                db.add_sale_purchase,
                int(product.get("id", 0)),
                int(user.id),
                username,
                str(product.get("channel_id", "")),
                price,
                "sent",
                "DM sent",
            )
            await interaction.response.send_message(
                f"✅ ส่งรายละเอียดสินค้าไปที่ DM แล้ว (#{purchase_id})",
                ephemeral=True,
            )
            ...
            await interaction.response.send_message(
                "❌ ส่ง DM ไม่สำเร็จ กรุณาเปิดรับข้อความส่วนตัวจากเซิร์ฟเวอร์นี้",
                ephemeral=True,
            )

def _format_options_text(text: str) -> str:
    lines = []
...
def _build_sale_embed(product: Dict) -> discord.Embed:
    title = str(product.get("name") or "สินค้า")
    category = str(product.get("category_name") or "")
    detail = str(product.get("detail") or "")
    form_detail = str(product.get("form_detail") or "")
    options_text = _format_options_text(product.get("options_text") or "")
    price = _safe_float(product.get("price", 0))
    embed = discord.Embed(
        title=title,
        description=detail[:4000] if detail else "",
        color=discord.Color.blurple(),
    )
    if category:
        embed.add_field(name="หมวดหมู่", value=category[:1024], inline=True)
    embed.add_field(name="ราคา", value=f"{price:.2f} บาท", inline=True)
    if product.get("inbound_id"):
        embed.add_field(name="Inbound ID", value=str(product.get("inbound_id")), inline=True)
    if product.get("form_url"):
        embed.add_field(name="URL ฟอร์ม", value=str(product.get("form_url"))[:1024], inline=False)
    if product.get("xui_url"):
        embed.add_field(name="URL 3x-ui", value=str(product.get("xui_url"))[:1024], inline=False)
    if form_detail:
        embed.add_field(name="รายละเอียดฟอร์ม", value=form_detail[:1024], inline=False)
    if options_text:
        embed.add_field(name="ตัวเลือกเพิ่มเติม", value=options_text[:1024], inline=False)
    if product.get("image_url"):
        embed.set_image(url=str(product.get("image_url")))
    embed.set_footer(text=f"ช่อง {product.get('channel_id','')}")
    return embed

async def _dispatch_sale_card(queue_row: Dict) -> None:
    product_id = int(queue_row.get("product_id", 0))
    product = await asyncio.to_thread(db.get_sale_product, product_id)
    if not product:
        await asyncio.to_thread(db.mark_sale_card_dispatched, int(queue_row["id"]), False, "product not found")
        return

    channel_id = str(queue_row.get("channel_id") or product.get("channel_id") or "").strip()
    if not channel_id:
        await asyncio.to_thread(db.mark_sale_card_dispatched, int(queue_row["id"]), False, "missing channel id")
        return

    category = None
    if product.get("category_id") is not None:
        category = await asyncio.to_thread(db.get_sale_category, int(product["category_id"]))
    product["category_name"] = category["name"] if category else ""
    embed = _build_sale_embed(product)
    view = SaleCardView(product)
    channel = None
    try:
    ...
    if channel is None:
        try:
            ...
            await asyncio.to_thread(db.mark_sale_card_dispatched, int(queue_row["id"]), False, f"channel error: {exc}")
            return

    try:
        await channel.send(embed=embed, view=view)
        await asyncio.to_thread(db.mark_sale_card_dispatched, int(queue_row["id"]), True, "sent")
    except Exception as exc:
        await asyncio.to_thread(db.mark_sale_card_dispatched, int(queue_row["id"]), False, str(exc))

@tasks.loop(seconds=SALE_DISPATCH_INTERVAL_SECONDS)
async def sale_dispatch_worker():
    try:
        jobs = await asyncio.to_thread(db.claim_pending_sale_cards, 5)
        for job in jobs:
            await _dispatch_sale_card(job)
    except Exception:
        logger.exception("sale dispatch worker failed")
...
async def on_ready():
    ...
    if not sale_dispatch_worker.is_running():
        sale_dispatch_worker.start()
    with suppress(Exception):
...
async def cmd_help(ctx: commands.Context):
    text = (
        ...
        f"พิมพ์ {MENU_COMMAND} เพื่อเปิดเมนูควบคุมบน VPS\n"
        f"พิมพ์ !menubot เพื่อเข้าจัดการเว็บไซต์จาก Discord ได้\n"
    )
    await ctx.send(text)
...
def _web_panel_path() -> str:
    path = str(os.getenv("WEB_PANEL_PATH", "/panel")).strip() or "/panel"
...
def _web_port() -> int:
    try:
        return int(os.getenv("WEB_PORT", "2026"))
    except Exception:
        return 2026


def _web_url(host: str | None = None) -> str:
    host = (host or "").strip() or "127.0.0.1"
    return f"http://{host}:{_web_port()}{_web_panel_path()}"


async def ask_text_from_channel(channel, author_id: int, prompt: str, *, flow: str, timeout: int = 120) -> Optional[str]:
    await channel.send(prompt)
    token = flow_current(author_id, flow)
    ...
    def check(message: discord.Message) -> bool:
        return (
            ...
            and not message.author.bot
            and flow_current(author_id, flow) == token
        )

    try:
        message = await bot.wait_for("message", check=check, timeout=timeout)
    except asyncio.TimeoutError:
        await channel.send("⌛ หมดเวลาแล้ว ลองใหม่อีกครั้ง")
        return None

    content = (message.content or "").strip()
    if content.lower() in {"cancel", "/cancel", "!cancel"}:
        await channel.send("⛔ ยกเลิกแล้ว")
        return None
    return content


async def run_webctl(*args: str, timeout: int = 240) -> tuple[int, str, str]:
    if not WEBCTL_SCRIPT.exists():
        return 1, "", f"ไม่พบสคริปต์ควบคุมเว็บ: {WEBCTL_SCRIPT}"
    cmd = " ".join(["bash", shlex.quote(str(WEBCTL_SCRIPT))] + [shlex.quote(a) for a in args])
    return await run_shell_command(cmd, cwd=APP_DIR, timeout=timeout)


def _trim_output(stdout: str, stderr: str) -> str:
    text = (stdout or "").strip() or (stderr or "").strip()
    return text or "ไม่มีข้อความตอบกลับ"


async def _web_status_text() -> str:
    code, stdout, stderr = await run_webctl("show", timeout=180)
    if code != 0 and not stdout.strip():
        return f"❌ ตรวจสอบเว็บไม่สำเร็จ\n{stderr.strip() or stdout.strip() or code}"
    return stdout.strip() or stderr.strip() or "ไม่พบข้อมูล"


async def _run_web_action(channel, author_id: int, action: str) -> None:
    code, stdout, stderr = await run_webctl(action, timeout=300)
    text = _trim_output(stdout, stderr)
    if code == 0:
        await channel.send(f"✅ {action}\n```text\n{text[:1800]}\n```")
    else:
        await channel.send(f"❌ {action} ล้มเหลว\n```text\n{text[:1800]}\n```")


async def _web_add_domain(channel, author_id: int) -> None:
    domain = await ask_text_from_channel(channel, author_id, "🌐 พิมพ์โดเมนที่จะผูกกับเว็บ (เช่น example.com) หรือพิมพ์ cancel เพื่อยกเลิก", flow="web:add-domain", timeout=120)
    if not domain:
        return
    code, stdout, stderr = await run_webctl("add-domain", domain.strip(), timeout=900)
    text = _trim_output(stdout, stderr)
    if code == 0:
        await channel.send(f"✅ ผูกโดเมนเรียบร้อย\n```text\n{text[:1800]}\n```")
    else:
        await channel.send(f"❌ ผูกโดเมนไม่สำเร็จ\n```text\n{text[:1800]}\n```")


async def _web_set_port(channel, author_id: int) -> None:
    port = await ask_text_from_channel(channel, author_id, "🔢 พิมพ์พอร์ตเว็บไซต์ใหม่ (1-65535) หรือ cancel", flow="web:set-port", timeout=120)
    if not port:
        return
    code, stdout, stderr = await run_webctl("set-port", port.strip(), timeout=300)
    text = _trim_output(stdout, stderr)
    if code == 0:
        await channel.send(f"✅ เปลี่ยนพอร์ตเว็บเรียบร้อย\n```text\n{text[:1800]}\n```")
    else:
        await channel.send(f"❌ เปลี่ยนพอร์ตเว็บไม่สำเร็จ\n```text\n{text[:1800]}\n```")


async def run_web_menu(channel, author) -> None:
    ...
    menu = (
        ...
        "7. เพิ่มโดเมน\n"
        "0. ย้อนกลับ\n\n"
        "พิมพ์เลข 1-7 หรือ `cancel` เพื่อยกเลิก"
    )
    choice = await ask_text_from_channel(channel, author.id, menu, flow="web:choose", timeout=120)
    if choice is None:
        return
    choice = choice.strip()

    if choice == "1":
        await channel.send(await _web_status_text())
        return
    if choice == "2":
        await _run_web_action(channel, author.id, "reset")
        return
    if choice == "3":
        await _web_set_port(channel, author.id)
        return
    if choice == "4":
        await _run_web_action(channel, author.id, "stop")
        return
    if choice == "5":
        await _run_web_action(channel, author.id, "start")
        return
    if choice == "6":
        await _run_web_action(channel, author.id, "restart")
        return
    if choice == "7":
        await _web_add_domain(channel, author.id)
        return

    await channel.send("⛔ กลับสู่เมนูหลัก")
    return
...
async def run_menubot_flow(channel, author) -> None:
    ...
    menu = (
        ...
        "5. อัปเดตสคริประบบ\n"
        "6. จัดการเว็บไซต์\n\n"
        "พิมพ์เลข 1-6 หรือ `cancel` เพื่อยกเลิก"
    )
    ...
    if choice == "1":
        ...
        cmd = (
            ...
            f"rm -f /etc/systemd/system/{SERVICE_NAME}.service /etc/systemd/system/{WEB_SERVICE_NAME}.service; systemctl daemon-reload; "
    ...
    if choice == "5":
        await channel.send("⬆️ กำลังตรวจสอบ Release ล่าสุด...")
        try:
            current_version = read_app_version()
            latest_version, _ = await asyncio.to_thread(get_latest_release_info)
        except Exception as exc:
            await channel.send(f"❌ อัปเดตไม่สำเร็จ: {exc}")
            return

        await channel.send(
            "📦 ตรวจสอบเวอร์ชั่นเรียบร้อย\n"
            f"เวอร์ชั่นก่อนอัปเดต: {current_version}\n"
            f"เวอร์ชั่นที่ตรวจพบ: {latest_version}"
        )

        if current_version == latest_version:
            await channel.send("✅ ตอนนี้เป็นเวอร์ชั่นล่าสุดอยู่แล้ว")
            return

        try:
            await asyncio.to_thread(_apply_latest_release_update)
        except Exception as exc:
            await channel.send(f"❌ ดาวน์โหลด/อัปเดต Release ไม่สำเร็จ: {exc}")
            return

        code, stdout, stderr = await _ensure_virtualenv()
        if code != 0:
            await channel.send(f"⚠️ ติดตั้ง dependencies ไม่สำเร็จ: {stderr.strip() or stdout.strip() or code}")
            return

        await run_shell_command(f"systemctl restart {shlex.quote(WEB_SERVICE_NAME)}", cwd=APP_DIR, timeout=30)
        await run_shell_command(f"systemctl restart {safe_service}", cwd=APP_DIR, timeout=30)
        await channel.send("✅ อัปเดตสคริประบบเรียบร้อย")
        return

    if choice == "6":
        await run_web_menu(channel, author)
        return

    await channel.send("❌ เลือกไม่ถูกต้อง")
@bot.command(name=MENU_COMMAND)
...
async def on_message(message: discord.Message):
    if message.author.bot:
        return
    if str(message.content or "").strip().lower() == MENU_COMMAND.lower():
