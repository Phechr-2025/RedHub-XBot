...
def _web_http_port() -> int:
    try:
        return int(os.getenv("WEB_HTTP_PORT", "9090"))
    except Exception:
        return 9090


def _web_https_port() -> int:
    try:
        return int(os.getenv("WEB_HTTPS_PORT", "9943"))
    except Exception:
        return 9943
...
def _web_url(host: str | None = None) -> str:
    ...
    return f"http://{host}:{_web_http_port()}{_web_panel_path()}"
...
async def _web_set_http_port(channel, author_id: int) -> None:
    port = await ask_text_from_channel(channel, author_id, "🌐 พิมพ์พอร์ต HTTP ใหม่ (1-65535) หรือ cancel", flow="web:set-http-port", timeout=120)
    if not port:
        return
    code, stdout, stderr = await run_webctl("set-http-port", port.strip(), timeout=300)
    text = _trim_output(stdout, stderr)
    if code == 0:
        await channel.send(f"✅ เปลี่ยนพอร์ต HTTP เรียบร้อย\n```text\n{text[:1800]}\n```")
    else:
        await channel.send(f"❌ เปลี่ยนพอร์ต HTTP ไม่สำเร็จ\n```text\n{text[:1800]}\n```")


async def _web_set_https_port(channel, author_id: int) -> None:
    port = await ask_text_from_channel(channel, author_id, "🔒 พิมพ์พอร์ต HTTPS ใหม่ (1-65535) หรือ cancel", flow="web:set-https-port", timeout=120)
    if not port:
        return
    code, stdout, stderr = await run_webctl("set-https-port", port.strip(), timeout=300)
    text = _trim_output(stdout, stderr)
    if code == 0:
        await channel.send(f"✅ เปลี่ยนพอร์ต HTTPS เรียบร้อย\n```text\n{text[:1800]}\n```")
    else:
        await channel.send(f"❌ เปลี่ยนพอร์ต HTTPS ไม่สำเร็จ\n```text\n{text[:1800]}\n```")
...
async def run_web_menu(channel, author) -> None:
    ...
    menu = (
        "🌐 จัดการเว็บไซต์\n"
        "1. ดู URL / สถานะพอร์ตที่เว็บไซต์ใช้\n"
        "2. สุ่ม Panel path ใหม่\n"
        "3. ตั้งค่า HTTP port\n"
        "4. ตั้งค่า HTTPS port\n"
        "5. ตั้งพอร์ตเว็บไซต์ภายใน\n"
        "6. หยุดเว็บ\n"
        "7. เริ่มเว็บ\n"
        "8. รีสตาร์ทเว็บ\n"
        "9. เพิ่มโดเมน\n"
        "0. ย้อนกลับ\n\n"
        "พิมพ์เลข 1-9 หรือ `cancel` เพื่อยกเลิก"
    )
    ...
    if choice == "2":
        ...
        return
    if choice == "3":
        await _web_set_http_port(channel, author.id)
        return
    if choice == "4":
        await _web_set_https_port(channel, author.id)
        return
    if choice == "5":
