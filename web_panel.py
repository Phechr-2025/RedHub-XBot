...
BOT_LOG_LIMIT = int(os.getenv("BOT_LOG_LIMIT", "200"))
BOT_STATUS_RESET_FILE = Path(os.getenv("BOT_STATUS_RESET_FILE", "/tmp/redhub-xbot-bot-status-reset"))
...
KEY_FIELDS: List[Tuple[str, str, str, str]] = [
    ...
    ("WEB_HTTP_PORT", "HTTP port", "number", "พอร์ตภายนอกสำหรับ HTTP"),
    ("WEB_HTTPS_PORT", "HTTPS port", "number", "พอร์ตภายนอกสำหรับ HTTPS"),
...
def _env() -> Dict[str, str]:
    ...
    data.setdefault("WEB_HTTP_PORT", "9090")
    data.setdefault("WEB_HTTPS_PORT", "9943")
...
def _panel_url(env: Dict[str, str], host: str) -> str:
    port = env.get("WEB_HTTP_PORT", "9090")
...
def _https_url(env: Dict[str, str]) -> str:
    ...
    if not domain:
        return ""
    path = _panel_path(env)
    https_port = env.get("WEB_HTTPS_PORT", "9943")
    return f"https://{domain}:{https_port}{path}"


def _bot_service_name(env: Dict[str, str]) -> str:
    return env.get("SERVICE_NAME", env.get("APP_SLUG", "xbot")) or "xbot"


def _bot_status_reset_stamp() -> float:
    try:
        return float(BOT_STATUS_RESET_FILE.read_text(encoding="utf-8").strip())
    except Exception:
        return 0.0


def _set_bot_status_reset_stamp() -> None:
    BOT_STATUS_RESET_FILE.parent.mkdir(parents=True, exist_ok=True)
    BOT_STATUS_RESET_FILE.write_text(str(time.time()), encoding="utf-8")


def _bot_journal_lines(service: str, since_ts: float, limit: int = BOT_LOG_LIMIT) -> List[str]:
    since_arg = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(since_ts)) if since_ts > 0 else "1 hour ago"
    cmd = [
        "journalctl",
        "-u",
        service,
    ...
    try:
        ...
        text_out = (proc.stdout or proc.stderr or "").strip()
        if text_out:
            lines = [line for line in text_out.splitlines() if line.strip()]
            return lines[-limit:]
    except Exception:
        pass
...
def _bot_status_payload(env: Dict[str, str]) -> Dict[str, object]:
    service = _bot_service_name(env)
    status = _service_status(service)
    active = status == "running"
    lines = _bot_journal_lines(service, _bot_status_reset_stamp(), BOT_LOG_LIMIT)
    return {
        "service": service,
        "status": status,
        "active": active,
        "log_count": len(lines),
        "logs": lines,
        "reset_at": _bot_status_reset_stamp(),
    }
...
def _render_page(env: Dict[str, str], note: str = "", level: str = "ok") -> str:
    ...
    bot_status_section = f"""
      <section class="card general-card">
        <div class="card-head">
          <div>
            <h3>สถานะบอท</h3>
            <p class="subtle">ดู log ล่าสุดตามเวลาและกดรีเซ็ตจุดเริ่มต้น log ได้</p>
          </div>
        </div>
        <div class="card-body">
          <div class="service-icon-row" style="margin-bottom:14px;">
            <div class="chip"><span class="chip-label">Service</span><span class="chip-status" id="bot-status-state">-</span></div>
            <div class="chip"><span class="chip-label">Logs</span><span class="chip-status" id="bot-status-count">0</span></div>
            <div class="chip"><span class="chip-label">Reset</span><span class="chip-status" id="bot-status-reset">-</span></div>
          </div>
          <div class="form-actions" style="margin-top:0;">
            <form method="post" action="{html.escape(panel_path)}/bot-status/reset" class="service-control-form" style="width:auto;">
              <button class="btn secondary" type="submit">รีเซ็ตสถานะ</button>
            </form>
          </div>
          <div class="chart-placeholder" style="margin-top:14px; align-items:stretch; justify-content:stretch; padding:16px;">
            <pre id="bot-log-body" style="margin:0; white-space:pre-wrap; word-break:break-word; max-height:540px; overflow:auto; font-size:12px; line-height:1.55; color:#dbeafe; width:100%;"></pre>
          </div>
        </div>
      </section>
    """
...
<body>
          ...
          <div class="general-wrap">
            <section class="card general-card">
              <div class="card-head">
                <div>
                  <h3>สถานะบอท</h3>
                  <p class="subtle">แสดง log การทำงานล่าสุดแบบเรียงเวลา และกดรีเซ็ตสถานะได้</p>
                </div>
              </div>
              <div class="card-body">
                <div class="service-icon-row" style="margin-bottom:14px;">
  ...
  <script>
    ...
    const tabMeta = {{
      ...
      'bot-status': {{
        title: 'สถานะบอท',
        subtitle: 'ดู log การทำงานและรีเซ็ตสถานะล่าสุด',
      }},
      general: {{
        title: 'ตั้งค่าทั่วไป',
    ...
    async function refreshBotStatus() {{
      const target = document.getElementById('bot-log-body');
      try {{
        ...
        const stateEl = document.getElementById('bot-status-state');
        const countEl = document.getElementById('bot-status-count');
        const resetEl = document.getElementById('bot-status-reset');
        if (stateEl) stateEl.textContent = data.status || '-';
        if (countEl) countEl.textContent = String(data.log_count ?? 0);
        if (resetEl) resetEl.textContent = data.reset_at ? new Date(data.reset_at * 1000).toLocaleString() : '-';
        if (target) target.textContent = (data.logs || []).join('\n');
...
def _handle_save(form: Dict[str, str]) -> Tuple[str, str]:
    ...
    if not updates.get("WEB_HTTP_PORT"):
        updates["WEB_HTTP_PORT"] = env.get("WEB_HTTP_PORT", "9090")
...
def main() -> None:
    env = _env()
    domain = str(env.get("WEB_DOMAIN", "") or "").strip()
    if domain:
        port = int(env.get("WEB_PORT", "2026") or 2026)
    else:
        port = int(env.get("WEB_HTTP_PORT", "9090") or 9090)
    host = "0.0.0.0"
