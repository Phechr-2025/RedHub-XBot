      ...
      <section class="card">
        ...
        <div class="card-body">
          <div class="stats-grid compact">
            {_stat_card("cpu", "CPU", float(stats["cpu_percent"]), f"<strong>CPU:</strong> {html.escape(stats['cpu_cores'])}", "blue")}
            {_stat_card("ram", "RAM", float(stats["mem_percent"]), f"<strong>RAM:</strong> {html.escape(stats['mem_used'])} / {html.escape(stats['mem_total'])}", "violet")}
            {_stat_card("storage", "Storage", float(stats["disk_percent"]), f"<strong>Storage:</strong> {html.escape(stats['disk_used'])} / {html.escape(stats['disk_total'])}", "blue")}
            {_stat_card("swap", "Swap", float(stats["swap_percent"]), f"<strong>Swap:</strong> {html.escape(stats['swap_used'])} / {html.escape(stats['swap_total'])}", "gray")}
          </div>
        </div>
      </section>
    """
...
<body>
  ...
  <script>
    ...
    function setMetric(metric, payload) {{
      ...
      if (node.value) {{
        if (metric === 'cpu') {{
          node.value.innerHTML = '<strong>CPU:</strong> ' + ((payload && payload.cores) || '');
        }} else {{
          const used = (payload && payload.used) || '';
          const total = (payload && payload.total) || '';
          const label = metric === 'storage' ? 'Storage' : metric === 'ram' ? 'RAM' : 'Swap';
          node.value.innerHTML = '<strong>' + label + ':</strong> ' + used + ' / ' + total;
        }}
      }}
    }}
