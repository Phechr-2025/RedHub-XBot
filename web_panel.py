...
BOT_STATUS_LOG_LIMIT = int(os.getenv("BOT_STATUS_LOG_LIMIT", "300"))
BOT_STATUS_RESET_FILE = Path(os.getenv("BOT_STATUS_RESET_FILE", "/tmp/redhub_xbot_bot_status_since"))
...
def _service_enabled(name: str) -> str:
    try:
        ...
        status = (proc.stdout.strip() or proc.stderr.strip() or "unknown").strip()
        if "System has not been booted" in status or "Failed to connect" in status:
            return "unknown"
...
def _bot_status_marker_path() -> Path:
    return BOT_STATUS_RESET_FILE


def _bot_status_reset_marker() -> str:
    marker = datetime.now(timezone.utc).isoformat()
    path = _bot_status_marker_path()
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(marker, encoding="utf-8")
    return marker


def _bot_status_since() -> str:
    path = _bot_status_marker_path()
    if not path.exists():
        return ""
    try:
...
def _journal_entries(service: str, since: str = "", limit: int = BOT_STATUS_LOG_LIMIT) -> List[Dict[str, str]]:
    cmd = ["journalctl", "-u", service, "--no-pager", "-o", "json"]
    if since:
        cmd += ["--since", since]
    else:
        cmd += ["-n", str(max(1, int(limit)))]
    try:
    ...
    for raw in proc.stdout.splitlines():
        ...
        try:
            if ts_raw:
                stamp = datetime.fromtimestamp(int(ts_raw) / 1_000_000, tz=timezone.utc).astimezone().strftime("%Y-%m-%d %H:%M:%S")
        except Exception:
            stamp = ""

        ident = str(data.get("SYSLOG_IDENTIFIER", "")).strip()
        unit = str(data.get("_SYSTEMD_UNIT", "")).strip()
        pid = str(data.get("_PID", "")).strip()
        source = ident or unit or service
        if pid:
            source = f"{source} pid {pid}"
...
def _bot_status_payload(env: Dict[str, str]) -> Dict[str, object]:
    service = env.get("SERVICE_NAME", "xbot")
    web_service = env.get("WEB_SERVICE_NAME", f"{service}-web")
    since = _bot_status_since()
    entries = _journal_entries(service, since=since, limit=BOT_STATUS_LOG_LIMIT)
    return {
        "service": service,
        "web_service": web_service,
        "active": _service_status(service),
        "enabled": _service_enabled(service),
        "since": since,
        "reset_file": str(_bot_status_marker_path()),
        "entries": entries,
        "count": len(entries),
    }
...
def _render_bot_status_section(env: Dict[str, str]) -> str:
    payload = _bot_status_payload(env)
    entries = payload.get("entries", [])
    log_lines: List[str] = []
    for item in entries:
        stamp = str(item.get("time", "")).strip() or "ไม่ทราบเวลา"
        source = str(item.get("source", "")).strip() or "unknown"
        message = str(item.get("message", "")).strip()
        log_lines.append(f"{stamp} : [{source}] {message}")
    if not log_lines:
        log_lines = ["ยังไม่มี log หลังการรีเซ็ต"]

    active = str(payload.get("active", "unknown"))
    enabled = str(payload.get("enabled", "unknown"))
    since = str(payload.get("since", "")).strip() or "ยังไม่ได้รีเซ็ต"
    ...
    return f"""
      <section class="card bot-status-card">
        <div class="card-head">
          <div>
            <h3>สถานะบอท</h3>
            <p class="subtle">แสดง log การทำงานแบบเรียลไทม์ล่าสุดอยู่ด้านล่าง</p>
          </div>
          <span class="pill pill-soft">Live</span>
        </div>
        <div class="card-body bot-status-body">
          <div class="status-grid">
            <div class="status-mini-card">
              <span class="status-mini-label">Service</span>
              <strong>{html.escape(payload.get("service", "xbot"))}</strong>
              <small class="subtle">status: <span id="bot-active-chip" class="chip-status {html.escape(active)}">{html.escape(active)}</span></small>
            </div>
            <div class="status-mini-card">
              <span class="status-mini-label">Enabled</span>
              <strong id="bot-enabled-value">{html.escape(enabled)}</strong>
              <small class="subtle">รีเซ็ตล่าสุด: <span id="bot-since-value">{html.escape(since)}</span></small>
            </div>
            <div class="status-mini-card">
              <span class="status-mini-label">Log</span>
              <strong id="bot-log-count">{count}</strong>
              <small class="subtle">รายการ log ที่แสดง</small>
            </div>
          </div>

          <div class="status-toolbar">
            <form method="post" action="{html.escape(_panel_path(env))}/bot-status/reset" class="service-control-form">
              <button class="service-control-btn danger" type="submit">รีเซ็ตสถานะ</button>
            </form>
            <button class="btn secondary" id="bot-status-refresh" type="button">รีเฟรช log</button>
          </div>

          <div class="log-shell">
            <div class="log-head">
              <span>Log ล่าสุด</span>
              <span class="muted" id="bot-log-updated">อัปเดตล่าสุด: {datetime.now().astimezone().strftime("%Y-%m-%d %H:%M:%S")}</span>
            </div>
            <pre class="log-view" id="bot-log-view">{html.escape(chr(10).join(log_lines))}</pre>
          </div>
        </div>
      </section>
    """
...
def _render_page(env: Dict[str, str], note: str = "", level: str = "ok") -> str:
    ...
    bot_status_section = _render_bot_status_section(env)
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    @media (max-width: 1100px) {{
      ...
      #sidebar-toggle:checked ~ .layout .overlay {{
        display: block;
        position: fixed;
        inset: 0;
      ...
      .stats-grid, .form-grid, .status-grid {{ grid-template-columns: 1fr; }}
...
<body>
  ...
  <div class="layout">
    <aside class="sidebar" id="sidebar">
      <div class="brand">
        ...
        <button class="sidebar-close" id="sidebar-close" type="button" aria-label="ปิดเมนู">✕</button>
      </div>
      ...
      <nav class="nav">
        ...
        <button data-tab="bot-status" type="button">
          <span class="nav-ico">▣</span>
          <span>สถานะบอท</span>
        </button>
    ...
    <main class="main">
      <div class="topbar">
        <button class="burger" id="burger" type="button" aria-label="เปิดเมนู" aria-controls="sidebar" aria-expanded="false">
  ...
  <script>
    ...
    const sections = {{
      ...
      'bot-status': document.getElementById('section-bot-status'),
    ...
    const tabMeta = {{
      ...
      'bot-status': {{
        title: 'สถานะบอท',
        subtitle: 'ดู log การรันของบอทแบบเรียลไทม์',
      }},
      general: {{
        title: 'ตั้งค่าทั่วไป',
    ...
    const botStatusNodes = {{
      activeChip: document.getElementById('bot-active-chip'),
      enabledValue: document.getElementById('bot-enabled-value'),
      sinceValue: document.getElementById('bot-since-value'),
      logCount: document.getElementById('bot-log-count'),
      logUpdated: document.getElementById('bot-log-updated'),
      logView: document.getElementById('bot-log-view'),
      refreshButton: document.getElementById('bot-status-refresh'),
    }};
    ...
    function renderBotStatus(data, forceScroll = false) {{
      const entries = Array.isArray(data && data.entries) ? data.entries : [];
      const lines = entries.map((item) => {{
        const stamp = (item && item.time) ? String(item.time) : 'ไม่ทราบเวลา';
      ...
      const content = lines.length ? lines.join('\n') : 'ยังไม่มี log หลังการรีเซ็ต';

      const logView = botStatusNodes.logView;
      const shouldStickToBottom = logView ? (logView.scrollTop + logView.clientHeight >= logView.scrollHeight - 40) : true;
      if (logView) {{
        logView.textContent = content;
        if (forceScroll || shouldStickToBottom) {{
          logView.scrollTop = logView.scrollHeight;
        }}
      }}

      if (botStatusNodes.activeChip) {{
        botStatusNodes.activeChip.className = `chip-status ${{(data && data.active) || 'unknown'}}`;
        botStatusNodes.activeChip.textContent = (data && data.active) || 'unknown';
      }}
      if (botStatusNodes.enabledValue) {{
        botStatusNodes.enabledValue.textContent = (data && data.enabled) || 'unknown';
      }}
      if (botStatusNodes.sinceValue) {{
        botStatusNodes.sinceValue.textContent = (data && data.since) || 'ยังไม่ได้รีเซ็ต';
      }}
      if (botStatusNodes.logCount) {{
        botStatusNodes.logCount.textContent = String(entries.length);
      }}
      if (botStatusNodes.logUpdated) {{
        botStatusNodes.logUpdated.textContent = 'อัปเดตล่าสุด: ' + new Date().toLocaleString('th-TH');
      }}
    }}

    async function refreshBotStatus(forceScroll = false) {{
      try {{
        ...
        if (!res.ok) return;
        const data = await res.json();
        renderBotStatus(data, forceScroll);
      }} catch (err) {{
        // ignore
      }}
    }}

    if (botStatusNodes.refreshButton) {{
      botStatusNodes.refreshButton.addEventListener('click', () => refreshBotStatus(true));
    }}
    ...
    function setTab(tab) {{
      ...
      if (target === 'bot-status') {{
        refreshBotStatus(true);
      }}
    }}

    function syncSidebarState(open) {{
      if (sidebarToggle) sidebarToggle.checked = !!open;
    ...
    if (sidebarToggle) {{
      sidebarToggle.addEventListener('change', () => {{
        ...
        if (burger) burger.setAttribute('aria-expanded', sidebarToggle.checked ? 'true' : 'false');
      }});
    }}
...
def application(environ, start_response):
    ...
    if path in {"/bot-status/reset", f"{panel_path}/bot-status/reset", f"{panel_path_alt}bot-status/reset"} and method == "POST":
        _bot_status_reset_marker()
        body = _render_page(_env(), note="รีเซ็ตสถานะบอทเรียบร้อยแล้ว", level="ok")
        start_response("200 OK", [("Content-Type", "text/html; charset=utf-8")])
        return [body.encode("utf-8")]

    if path in {"/bot-status", f"{panel_path}/bot-status", f"{panel_path_alt}bot-status"} and method == "GET":
        payload = json.dumps(_bot_status_payload(env), ensure_ascii=False).encode("utf-8")
        start_response(
            "200 OK",
            [
