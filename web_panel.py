...
def _panel_url(env: Dict[str, str], host: str) -> str:
    port = env.get("WEB_PORT", "8080")
    path = env.get("WEB_PANEL_PATH", DEFAULT_PANEL_PATH).strip() or DEFAULT_PANEL_PATH
...
def _render_notice(title: str, message: str, level: str = "ok") -> str:
    ...
    <section class="{cls}">
      ...
      <p>{message}</p>
      <p><a href="/panel">กลับไปหน้าเว็บ</a></p>
    </section>
    """
...
def _render_page(env: Dict[str, str], note: str = "", level: str = "ok") -> str:
    ...
    panel_path = env.get("WEB_PANEL_PATH", DEFAULT_PANEL_PATH)
...
def _handle_save(form: Dict[str, str]) -> Tuple[str, str]:
    ...
    if not updates.get("WEB_PORT"):
        updates["WEB_PORT"] = env.get("WEB_PORT", "8080")
...
def application(environ, start_response):
    ...
    if path in {"/", "/panel"} and method == "GET":
    ...
    if path == "/restart-bot" and method == "POST":
        _restart_after("bot")
        body = _render_page(_env(), note="สั่งรีสตาร์ท Bot service แล้ว", level="ok")
        start_response("200 OK", [("Content-Type", "text/html; charset=utf-8")])
        return [body.encode("utf-8")]

    if path == "/restart-web" and method == "POST":
        _restart_after("web")
...
def main() -> None:
    env = _env()
    port = int(env.get("WEB_PORT", "8080") or 8080)
    host = "0.0.0.0"
