...
def _control_service(name: str, action: str) -> None:
    action = action.lower().strip()
    if action not in {"start", "stop", "restart"}:
        return
    subprocess.run(["systemctl", action, name], check=False, timeout=30)
...
def _brand_mark() -> str:
    if BRAND_IMAGE_URL:
        src = html.escape(BRAND_IMAGE_URL, quote=True)
        alt = html.escape(BRAND_IMAGE_ALT)
        return f"""
        <img class="brand-image" src="{src}" alt="{alt}" referrerpolicy="no-referrer" loading="eager" decoding="async" crossorigin="anonymous">
        """
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .brand-image {{
      ...
      flex: 0 0 auto;
      background: linear-gradient(135deg, rgba(57,169,255,.2), rgba(124,58,237,.2));
    }}
    .brand h1 {{
      font-size: 17px;
      line-height: 1.2;
    ...
    .action-btn.web {{
      box-shadow: inset 0 0 0 1px rgba(57,169,255,.12);
    }}
    .action-btn.bot {{
    ...
    .action-btn.success {{
      box-shadow: inset 0 0 0 1px rgba(25,195,125,.16);
    }}
    .main {{
      min-width: 0;
      padding: 18px;
...
<body>
    ...
    <aside class="sidebar" id="sidebar">
      ...
      <div class="sidebar-footer">
        ...
        <div class="sidebar-stats">
          <div class="mini-stat" data-mini-metric="cpu">
            <span>CPU</span>
            <strong id="mini-cpu">—</strong>
          </div>
          <div class="mini-stat" data-mini-metric="ram">
            <span>RAM</span>
            <strong id="mini-ram">—</strong>
          </div>
          <div class="mini-stat" data-mini-metric="storage">
            <span>Storage</span>
            <strong id="mini-storage">—</strong>
          </div>
          <div class="mini-stat" data-mini-metric="swap">
            <span>Swap</span>
            <strong id="mini-swap">—</strong>
          </div>
        </div>

        <div class="sidebar-actions">
          <form method="post" action="{html.escape(panel_path)}/restart-web">
            <button class="action-btn web" type="submit">รีสตาร์ทเว็บ</button>
          </form>
          <form method="post" action="{html.escape(panel_path)}/bot-restart">
            <button class="action-btn bot" type="submit">รีสตาร์ทบอท</button>
          </form>
          <form method="post" action="{html.escape(panel_path)}/bot-stop">
            <button class="action-btn danger" type="submit">หยุดบอท</button>
          </form>
          <form method="post" action="{html.escape(panel_path)}/bot-start">
            <button class="action-btn success" type="submit">เริ่มบอท</button>
          </form>
        </div>
      </div>
    </aside>
    ...
    <main class="main">
      <div class="topbar">
        ...
        <div class="topbar-copy">
          ...
          <h2 id="page-title">แดชบอร์ดสถานะระบบ</h2>
          <p id="page-desc">ภาพรวมแบบสดของ CPU, RAM, Storage และ Swap</p>
        </div>
      </div>
  ...
  <script>
    ...
    function setMetric(metric, payload) {{
      ...
      const mini = document.getElementById(`mini-${{metric}}`);
      if (!node) return;
      ...
      if (mini) {{
        mini.textContent = (payload && payload.value) ? `${{payload.value}}` : '—';
      }}
    }}
    ...
    const pageMeta = {{
      overview: {{
        ...
        desc: 'ภาพรวมแบบสดของ CPU, RAM, Storage และ Swap',
      }},
      general: {{
        title: 'ตั้งค่าทั่วไป',
        desc: 'แก้ค่า .env และตัวแปรสำคัญในหน้าเดียว',
      }},
    }};
    ...
    function setTab(tab) {{
      ...
      if (descNode) descNode.textContent = meta.desc;
...
def application(environ, start_response):
    ...
    if path in {"/restart-web", f"{panel_path}/restart-web", f"{panel_path_alt}restart-web"} and method == "POST":
        _restart_after("web")
        body = _render_page(_env(), note="สั่งรีสตาร์ท Web service แล้ว", level="ok")
        start_response("200 OK", [("Content-Type", "text/html; charset=utf-8")])
        return [body.encode("utf-8")]

    if path in {"/bot-restart", f"{panel_path}/bot-restart", f"{panel_path_alt}bot-restart"} and method == "POST":
        env_now = _env()
        _control_service(env_now.get("SERVICE_NAME", "xbot"), "restart")
        body = _render_page(env_now, note="สั่งรีสตาร์ท Bot service แล้ว", level="ok")
        start_response("200 OK", [("Content-Type", "text/html; charset=utf-8")])
        return [body.encode("utf-8")]

    if path in {"/bot-stop", f"{panel_path}/bot-stop", f"{panel_path_alt}bot-stop"} and method == "POST":
        env_now = _env()
        _control_service(env_now.get("SERVICE_NAME", "xbot"), "stop")
        body = _render_page(env_now, note="สั่งหยุด Bot service แล้ว", level="warn")
        start_response("200 OK", [("Content-Type", "text/html; charset=utf-8")])
        return [body.encode("utf-8")]

    if path in {"/bot-start", f"{panel_path}/bot-start", f"{panel_path_alt}bot-start"} and method == "POST":
        env_now = _env()
        _control_service(env_now.get("SERVICE_NAME", "xbot"), "start")
        body = _render_page(env_now, note="สั่งเริ่ม Bot service แล้ว", level="ok")
        start_response("200 OK", [("Content-Type", "text/html; charset=utf-8")])
        return [body.encode("utf-8")]
