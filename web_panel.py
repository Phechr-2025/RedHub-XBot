...
def _stat_card(title: str, percent: float, value_line: str, accent: str = "blue") -> str:
    return f"""
      <div class="stat-card">
        <div class="gauge {accent}" style="{_ring_style(percent)}">
        ...
        <div class="stat-text">
          ...
          <div class="stat-value">{value_line}</div>
        </div>
      </div>
    """
...
def _render_page(env: Dict[str, str], note: str = "", level: str = "ok") -> str:
    ...
    overview_cards = f"""
      <section class="card hero-card">
        <div class="hero-top">
          <button class="burger-mini" type="button" id="burger" aria-label="เปิดเมนู">
        ...
        <div class="hero-url">URL: {html.escape(current_url)}</div>
      </section>
      ...
      <section class="card">
        <div class="card-head">
          <div>
            <h3>ภาพรวมระบบ</h3>
            <p class="subtle">สถิติคำนวณจากเครื่องจริงแบบเรียลไทม์</p>
          </div>
          <span class="pill">{html.escape(domain)}</span>
        </div>
        <div class="card-body">
          <div class="stats-grid">
            {_stat_card("CPU", float(stats["cpu_percent"]), f"<strong>CPU:</strong> {html.escape(stats['cpu_cores'])}", "blue")}
            {_stat_card("RAM", float(stats["mem_percent"]), f"<strong>RAM:</strong> {html.escape(stats['mem_used'])} / {html.escape(stats['mem_total'])}", "violet")}
            {_stat_card("Swap", float(stats["swap_percent"]), f"<strong>Swap:</strong> {html.escape(stats['swap_used'])} / {html.escape(stats['swap_total'])}", "gray")}
            {_stat_card("Storage", float(stats["disk_percent"]), f"<strong>Storage:</strong> {html.escape(stats['disk_used'])} / {html.escape(stats['disk_total'])}", "blue")}
          </div>
        </div>
      </section>
        ...
        <div class="card-head">
          <div class="service-head">
            <div class="service-title">
              ...
              <span class="version-tag">{html.escape(xray_version)}</span>
            </div>
            <span class="status-dot {html.escape('online' if bot_status == 'running' else 'offline')}"></span>
            <span class="service-status">{html.escape(xray_status.capitalize() if xray_status else 'Unknown')}</span>
          </div>
        </div>
        <div class="service-actions">
          <div class="service-icon-row">
            <button class="icon-btn" type="button" title="Power">⏻</button>
            <button class="icon-btn" type="button" title="Refresh">↻</button>
            <button class="icon-btn" type="button" title="Tools">🔧</button>
          </div>
        </div>
      </section>
      ...
      <section class="card service-card">
        <div class="card-head">
          <div class="service-head">
            <div class="service-title">
              <span class="service-name">Manage</span>
            </div>
          </div>
        </div>
        <div class="service-actions">
          <div class="service-icon-row">
            <button class="icon-btn" type="button" title="List">☰</button>
            <button class="icon-btn" type="button" title="Sliders">🎚</button>
            <button class="icon-btn" type="button" title="Lock">🔒</button>
          </div>
        </div>
      </section>
      ...
      <section class="card service-card">
        <div class="card-head">
          <div class="service-head">
            <div class="service-title">
              <span class="service-name">3X-UI</span>
              <span class="version-tag">{html.escape(xui_version)}</span>
            </div>
          </div>
        </div>
        <div class="service-actions">
          <div class="service-icon-row">
            <button class="icon-btn" type="button" title="Telegram">✈</button>
            <button class="icon-btn" type="button" title="Cloud">☁</button>
          </div>
        </div>
      </section>
      ...
      <section class="card">
        <div class="card-head">
          <div>
            <h3>Charts</h3>
            <p class="subtle">ข้อมูลสรุปและกราฟจะแสดงในส่วนนี้</p>
          </div>
        </div>
        <div class="card-body">
...
<head>
  ...
  <title>เว็บไซต์ตั้งค่าระบบ • RedHub-XBot</title>
  <style>
    :root {{
      --bg: #070b14;
      --bg2: #0b1220;
    ...
    * {{ box-sizing: border-box; }}
    ...
    @media (max-width: 640px) {{
      ...
      .hero-card {{ min-height: 500px; padding: 16px; }}
  ...
  <div class="layout">
    ...
    <main class="main">
      <div class="topbar">
        ...
        <span class="pill">Panel: {html.escape(panel_path)}</span>
      </div>
