...
def _shop_section(
    ...
    def _count_products_for_form(form_id_value: int) -> int:
        total = 0
        for cat in db.list_sale_categories(form_id=form_id_value, active_only=False):
            total += len(db.list_sale_products(category_id=int(cat["id"]), active_only=False))
        return total

    def _render_dashboard() -> str:
        forms = db.list_sale_forms(active_only=False)
        q_norm = q.strip().lower()
        if q_norm:
            forms = [
                f for f in forms
                if q_norm in str(f.get("name", "")).lower() or q_norm in str(f.get("description", "")).lower()
            ]

        form_cards = []
        for form in forms:
            fid = int(form.get("id", 0))
            cat_count = len(db.list_sale_categories(form_id=fid, active_only=False))
            prod_count = _count_products_for_form(fid)
            form_cards.append(f"""
              <section class="mini-card shop-form-card">
                <div class="mini-row">
                  <div>
                    <strong>{html.escape(form.get('name',''))}</strong>
                    <div class="subtle">{html.escape(form.get('description','') or 'ไม่มีคำอธิบาย')}</div>
                  </div>
                  <div class="row-actions">
                    <span class="pill">หมวด {cat_count}</span>
                    <span class="pill">สินค้า {prod_count}</span>
                  </div>
                </div>
                <div class="row-actions shop-card-actions">
                  <a class="btn tiny" href="{_qs(view='form_categories', form_id=fid)}">จัดการหมวดหมู่</a>
                  <a class="btn tiny" href="{_qs(view='form_new', form_id=fid)}">แก้ไขฟอร์ม</a>
                  <form method="post" action="/save">
                    <input type="hidden" name="action" value="shop_form_delete">
          ...
          <section class="card">
            <div class="card-head">
              <div>
                ...
                <p class="subtle">สร้างฟอร์ม แล้วค่อยเข้าไปจัดการหมวดหมู่และสินค้า</p>
              </div>
              <a class="btn" href="{_qs(view='form_new')}">+ สร้างฟอร์ม</a>
            </div>
            <div class="card-body">
              <form method="get" action="" class="shop-search">
                <input type="hidden" name="view" value="dashboard">
                <input type="search" name="q" value="{html.escape(q)}" placeholder="ค้นหาฟอร์ม...">
                <button class="btn" type="submit">ค้นหา</button>
              </form>
              <div class="subtle">ด้านล่างคือรายการฟอร์มตัวอย่างที่สร้างไว้แล้ว</div>
              <div class="stack shop-stack">
                {''.join(form_cards) if form_cards else '<div class="subtle">ยังไม่มีฟอร์ม</div>'}
              </div>
            </div>
          </section>
        """

    def _render_form_editor() -> str:
        form = db.get_sale_form(form_id) if form_id else None
        editing_id = int(form.get("id", 0)) if form else 0
        name = html.escape(form.get("name", "") if form else "")
        desc = html.escape(form.get("description", "") if form else "")
        sort_order = html.escape(str(form.get("sort_order", 0)) if form else "0")
        preview_name = form.get("name", "ตัวอย่างฟอร์ม") if form else "ตัวอย่างฟอร์ม"
        preview_desc = form.get("description", "หน้าตาเปล่าๆของฟอร์มตัวอย่าง") if form else "หน้าตาเปล่าๆของฟอร์มตัวอย่าง"
        return f"""
          <section class="card">
            <div class="card-head">
              <div>
                <h3>สร้างฟอร์มใหม่</h3>
                <p class="subtle">หน้าตัวอย่างฟอร์มแบบว่าง ๆ ก่อนเข้าไปจัดการหมวดหมู่</p>
              </div>
              <div class="row-actions">
                <a class="btn tiny" href="{_qs(view='dashboard')}">ย้อนกลับ</a>
                {'<a class="btn tiny" href="' + _qs(view='form_categories', form_id=editing_id) + '">จัดการหมวดหมู่</a>' if editing_id else ''}
              </div>
            </div>
            <div class="card-body">
              <div class="shop-preview">
                <div class="shop-preview-title">{html.escape(preview_name)}</div>
                <div class="subtle">{html.escape(preview_desc)}</div>
                <div class="shop-preview-empty">
                  <div class="shop-preview-chip"></div>
                  <div class="shop-preview-chip"></div>
                  <div class="shop-preview-chip"></div>
                  <div class="shop-preview-chip"></div>
                </div>
              ...
              <form method="post" action="/save">
                <input type="hidden" name="action" value="shop_form_save">
                <input type="hidden" name="form_id" value="{editing_id if editing_id else ''}">
                <div class="form-grid">
                  <label class="field"><span>ชื่อฟอร์ม</span><input name="form_name" type="text" value="{name}" placeholder="เช่น Netflix / Disney"></label>
                  <label class="field"><span>คำอธิบายฟอร์ม</span><input name="form_description" type="text" value="{desc}" placeholder="คำอธิบายฟอร์ม"></label>
                  <label class="field"><span>ลำดับการแสดง</span><input name="form_sort_order" type="number" value="{sort_order}"></label>
                </div>
                <div class="form-actions">
                  <button class="btn" type="submit">บันทึกฟอร์ม</button>
                </div>
              </form>
            </div>
          </section>
    ...
    def _render_form_categories() -> str:
        forms = db.list_sale_forms(active_only=False)
        current_form = db.get_sale_form(form_id) if form_id else (forms[0] if forms else None)
        current_form_id = int(current_form.get("id", 0)) if current_form else 0
        current_form_name = html.escape(current_form.get("name", "ฟอร์ม") if current_form else "ฟอร์ม")
        current_form_desc = html.escape(current_form.get("description", "") if current_form else "")
        categories = db.list_sale_categories(form_id=current_form_id, active_only=False) if current_form_id else []
        edit_cat = db.get_sale_category(edit_category_id) if edit_category_id else None
        if edit_cat and int(edit_cat.get("form_id", 0)) != current_form_id:
            edit_cat = None

        category_cards = []
        for cat in categories:
            cat_id = int(cat.get("id", 0))
            prod_count = len(db.list_sale_products(category_id=cat_id, active_only=False))
            category_cards.append(f"""
              <section class="mini-card shop-form-card">
                <div class="mini-row">
                  <div>
                    <strong>{html.escape(cat.get('name',''))}</strong>
                    <div class="subtle">{html.escape(cat.get('description','') or 'ไม่มีคำอธิบาย')}</div>
                  </div>
                  <div class="row-actions">
                    <span class="pill">สินค้า {prod_count}</span>
                  </div>
                ...
                <div class="row-actions shop-card-actions">
                  <a class="btn tiny" href="{_qs(view='category_products', form_id=current_form_id, category_id=cat_id)}">จัดการสินค้า</a>
                  <a class="btn tiny" href="{_qs(view='form_categories', form_id=current_form_id, edit_category_id=cat_id)}">แก้ไข</a>
                  <form method="post" action="/save">
        ...
        cat_form_id = current_form_id
        cat_id_value = int(edit_cat.get("id", 0)) if edit_cat else 0
        cat_name = html.escape(edit_cat.get("name", "") if edit_cat else "")
        cat_desc = html.escape(edit_cat.get("description", "") if edit_cat else "")
        cat_sort = html.escape(str(edit_cat.get("sort_order", 0)) if edit_cat else "0")

        return f"""
          <section class="card">
            <div class="card-head">
              <div>
                <h3>{current_form_name}</h3>
                <p class="subtle">{current_form_desc or 'หน้าจัดการหมวดหมู่ของฟอร์มนี้'}</p>
              </div>
              <div class="row-actions">
                <a class="btn tiny" href="{_qs(view='dashboard')}">กลับหน้าหลัก</a>
                <a class="btn tiny" href="{_qs(view='form_new', form_id=current_form_id)}">แก้ไขฟอร์ม</a>
                <a class="btn tiny" href="{_qs(view='form_new')}">+ สร้างฟอร์ม</a>
              </div>
            </div>
            <div class="card-body">
              <div class="shop-preview">
                <div class="shop-preview-title">ตัวอย่างฟอร์มขาย</div>
                <div class="subtle">กดที่ฟอร์มด้านล่างเพื่อเข้าไปจัดการหมวดหมู่</div>
              </div>

              <form method="post" action="/save" class="shop-inline-form">
                <input type="hidden" name="action" value="shop_category_save">
                <input type="hidden" name="category_form_id" value="{cat_form_id}">
                <input type="hidden" name="view" value="form_categories">
                <div class="form-grid">
                  <label class="field"><span>ID หมวดหมู่ (เว้นว่าง = สร้างใหม่)</span><input name="category_id" type="number" value="{cat_id_value if cat_id_value else ''}" placeholder="เช่น 1"><small>ใส่เพื่อแก้ไข</small></label>
                  <label class="field"><span>ค่าสร้างรายการหมวดหมู่</span><input name="category_name" type="text" value="{cat_name}" placeholder="เช่น Netflix"><small>ชื่อหมวดหมู่</small></label>
                  <label class="field"><span>คำอธิบายหมวดหมู่</span><input name="category_description" type="text" value="{cat_desc}" placeholder="รายละเอียดหมวดหมู่"><small>แสดงในหน้าเว็บ</small></label>
                  <label class="field"><span>ลำดับการแสดง</span><input name="category_sort_order" type="number" value="{cat_sort}"><small>น้อยขึ้นก่อน</small></label>
                </div>
                <div class="form-actions">
              ...
              <div class="stack shop-stack">
                {''.join(category_cards) if category_cards else '<div class="subtle">ยังไม่มีหมวดหมู่ในฟอร์มนี้</div>'}
              </div>
            </div>
          </section>
        """

    def _render_category_products() -> str:
        cat = db.get_sale_category(category_id) if category_id else None
        if not cat and form_id:
            cats = db.list_sale_categories(form_id=form_id, active_only=False)
            ...
            return """
              <section class="card">
                <div class="card-head">
                  <div>
                    <h3>ไม่พบหมวดหมู่</h3>
                    <p class="subtle">กรุณากลับไปเลือกฟอร์มหรือสร้างหมวดหมู่ก่อน</p>
                  </div>
                  <a class="btn tiny" href="?view=dashboard">กลับหน้าหลัก</a>
                </div>
              </section>
            """
        current_form = db.get_sale_form(int(cat.get("form_id", 0)))
        current_form_id = int(current_form.get("id", 0)) if current_form else int(cat.get("form_id", 0))
        current_category_id = int(cat.get("id", 0))
        products = db.list_sale_products(category_id=current_category_id, active_only=False)
        edit_product = db.get_sale_product(edit_product_id) if edit_product_id else None
        if edit_product and int(edit_product.get("category_id", 0)) != current_category_id:
            edit_product = None
              ...
              <section class="mini-card shop-form-card">
                <div class="mini-row">
                  <div>
                    ...
                    <div class="subtle">ราคา {html.escape(str(prod.get('price',0)))} บาท • ช่อง {html.escape(str(prod.get('channel_id','')))}</div>
                  </div>
                  <div class="row-actions">
                    <span class="pill">#{prod_id}</span>
                  </div>
                </div>
                <div class="row-actions shop-card-actions">
                  <a class="btn tiny" href="{_qs(view='category_products', form_id=current_form_id, category_id=current_category_id, edit_product_id=prod_id)}">แก้ไข</a>
                  <form method="post" action="/save">
        ...
        prod_id_value = int(edit_product.get("id", 0)) if edit_product else 0
        prod_name = html.escape(edit_product.get("name", "") if edit_product else "")
        prod_price = html.escape(str(edit_product.get("price", 0)) if edit_product else "0")
        prod_inbound = html.escape(str(edit_product.get("inbound_id", 0)) if edit_product else "0")
        prod_xui_url = html.escape(edit_product.get("xui_url", "") if edit_product else "")
        prod_form_url = html.escape(edit_product.get("form_url", "") if edit_product else "")
        prod_form_detail = html.escape(edit_product.get("form_detail", "") if edit_product else "")
        prod_detail = html.escape(edit_product.get("detail", "") if edit_product else "")
        prod_image_url = html.escape(edit_product.get("image_url", "") if edit_product else "")
        prod_channel_id = html.escape(edit_product.get("channel_id", "") if edit_product else "")
        prod_options = html.escape(edit_product.get("options_text", "") if edit_product else "")
        prod_sort_order = html.escape(str(edit_product.get("sort_order", 0)) if edit_product else "0")

        return f"""
          <section class="card">
            <div class="card-head">
              <div>
                <h3>จัดการสินค้า: {html.escape(cat.get('name',''))}</h3>
                <p class="subtle">ฟอร์ม: {html.escape(current_form.get('name','') if current_form else 'ไม่พบฟอร์ม')}</p>
              </div>
              <div class="row-actions">
                <a class="btn tiny" href="{_qs(view='form_categories', form_id=current_form_id, edit_category_id=current_category_id)}">กลับไปหมวดหมู่</a>
                <a class="btn tiny" href="{_qs(view='form_new', form_id=current_form_id)}">แก้ไขฟอร์ม</a>
              </div>
            </div>
            <div class="card-body">
              <form method="post" action="/save" class="shop-inline-form">
                ...
                <input type="hidden" name="product_id" value="{prod_id_value if prod_id_value else ''}">
                <input type="hidden" name="product_category_id" value="{current_category_id}">
                <input type="hidden" name="view" value="category_products">
                <div class="form-grid">
                  <label class="field"><span>ชื่อสินค้าที่จะขาย</span><input name="product_name" type="text" value="{prod_name}" placeholder="ชื่อสินค้า"></label>
                  <label class="field"><span>ราคา</span><input name="product_price" type="number" step="0.01" value="{prod_price}"><small>ราคาเป็นบาท</small></label>
                  <label class="field"><span>id Inbound</span><input name="product_inbound_id" type="number" value="{prod_inbound}"><small>หมายเลข inbound</small></label>
                  <label class="field"><span>url 3x-ui</span><input name="product_xui_url" type="text" value="{prod_xui_url}" placeholder="http://..."><small>URL ของ 3x-ui</small></label>
                  <label class="field"><span>url ตั้งรูปฟอร์ม</span><input name="product_form_url" type="text" value="{prod_form_url}" placeholder="https://..."><small>URL ฟอร์มหรือรูปฟอร์ม</small></label>
                  <label class="field"><span>รายละเอียดฟอร์ม</span><input name="product_form_detail" type="text" value="{prod_form_detail}" placeholder="รายละเอียดฟอร์ม"><small>รายละเอียดฟอร์มเพิ่มเติม</small></label>
                  <label class="field"><span>รายละเอียดสินค้า</span><input name="product_detail" type="text" value="{prod_detail}" placeholder="รายละเอียดสินค้า"><small>อธิบายสินค้า</small></label>
                  <label class="field"><span>url รูปปกสินค้า</span><input name="product_image_url" type="text" value="{prod_image_url}" placeholder="https://..."><small>รูปภาพสินค้า</small></label>
                  <label class="field"><span>ID ช่องเช่น 1241740147842748556</span><input name="product_channel_id" type="text" value="{prod_channel_id}" placeholder="1241740147842748556"><small>ช่องที่จะส่งการ์ด</small></label>
                  <label class="field"><span>ลำดับการแสดง</span><input name="product_sort_order" type="number" value="{prod_sort_order}"><small>น้อยขึ้นก่อน</small></label>
                  <label class="field full"><span>ตัวเลือกเพิ่ม</span><textarea name="product_options_text" rows="4" placeholder="เพิ่มบรรทัดละ 1 ข้อ">{prod_options}</textarea><small>ใส่ได้หลายบรรทัด</small></label>
                </div>
                <div class="form-actions">
              ...
              <div class="stack shop-stack">
                {''.join(product_cards) if product_cards else '<div class="subtle">ยังไม่มีสินค้าในหมวดหมู่นี้</div>'}
              </div>
            </div>
          </section>
        """
    ...
    if view == "form_new":
        return _render_form_editor()
    if view == "form_categories":
        return _render_form_categories()
    if view == "category_products":
        return _render_category_products()
    return _render_dashboard()
def _render_page(env: Dict[str, str], note: str = "", level: str = "ok", view: str = "dashboard", q: str = "", form_id: int = 0, category_id: int = 0, edit_category_id: int = 0, edit_product_id: int = 0) -> str:
    host = _public_ip()
    ...
    shop_section = _shop_section(view=view, q=q, form_id=form_id, category_id=category_id, edit_category_id=edit_category_id, edit_product_id=edit_product_id)
...
def _handle_shop_action(form: Dict[str, str]) -> Tuple[str, str, Dict[str, int | str]]:
    action = (form.get("action") or "").strip()
    meta: Dict[str, int | str] = {"view": "dashboard"}

    if action == "shop_form_save":
        name = (form.get("form_name") or "").strip()
        if not name:
            return ("กรุณาระบุชื่อฟอร์ม", "warn", meta)
        description = (form.get("form_description") or "").strip()
        sort_order = _safe_int(form.get("form_sort_order", "0"))
        form_id = form.get("form_id", "").strip()
        if form_id and form_id.isdigit():
            db.update_sale_form(int(form_id), name, description, sort_order, 1)
            meta.update({"view": "form_new", "form_id": int(form_id)})
            return ("อัปเดตฟอร์มเรียบร้อยแล้ว", "ok", meta)
        new_id = db.create_sale_form(name, description, sort_order)
        meta.update({"view": "form_new", "form_id": int(new_id)})
        return ("สร้างฟอร์มเรียบร้อยแล้ว", "ok", meta)

    if action == "shop_form_delete":
        form_id = form.get("form_id", "").strip()
        if not form_id.isdigit():
            return ("ไม่พบ form_id", "warn", meta)
        db.delete_sale_form(int(form_id))
        return ("ลบฟอร์มเรียบร้อยแล้ว", "ok", meta)

    if action == "shop_category_save":
        name = (form.get("category_name") or "").strip()
        if not name:
            return ("กรุณาระบุชื่อหมวดหมู่", "warn", meta)
        description = (form.get("category_description") or "").strip()
        sort_order = _safe_int(form.get("category_sort_order", "0"))
        form_id = _safe_int(form.get("category_form_id") or form.get("form_id") or "1", 1)
        cat_id = form.get("category_id", "").strip()
        if cat_id and cat_id.isdigit():
            db.update_sale_category(int(cat_id), name, description, sort_order, form_id, 1)
            meta.update({"view": "form_categories", "form_id": form_id, "edit_category_id": int(cat_id)})
            return ("อัปเดตหมวดหมู่เรียบร้อยแล้ว", "ok", meta)
        new_id = db.create_sale_category(name, description, sort_order, form_id, 1)
        meta.update({"view": "form_categories", "form_id": form_id, "edit_category_id": int(new_id)})
        return ("สร้างหมวดหมู่เรียบร้อยแล้ว", "ok", meta)

    if action == "shop_category_delete":
        cat_id = form.get("category_id", "").strip()
        form_id = _safe_int(form.get("form_id", "0"))
        if not cat_id.isdigit():
            return ("ไม่พบ category_id", "warn", meta)
        cat = db.get_sale_category(int(cat_id))
        if cat:
            form_id = int(cat.get("form_id", form_id or 0))
        db.delete_sale_category(int(cat_id))
        meta.update({"view": "form_categories", "form_id": form_id})
        return ("ลบหมวดหมู่เรียบร้อยแล้ว", "ok", meta)

    if action == "shop_product_save":
        category_id = _safe_int(form.get("product_category_id", "0"))
        name = (form.get("product_name") or "").strip()
        if not category_id or not name:
            return ("กรุณาเลือกหมวดหมู่และระบุชื่อสินค้า", "warn", meta)
        ...
        if product_id and product_id.isdigit():
            ...
            prod_id = db.create_sale_product(**payload)
        cat = db.get_sale_category(category_id)
        form_id = int(cat.get("form_id", 1)) if cat else 1
        meta.update({"view": "category_products", "form_id": form_id, "category_id": category_id, "edit_product_id": prod_id})
        return ("บันทึกรายการสินค้าเรียบร้อยแล้ว", "ok", meta)

    if action == "shop_product_delete":
        product_id = form.get("product_id", "").strip()
        category_id = _safe_int(form.get("category_id", "0"))
        if not product_id.isdigit():
            return ("ไม่พบ product_id", "warn", meta)
        prod = db.get_sale_product(int(product_id))
        if prod:
            category_id = int(prod.get("category_id", category_id or 0))
        cat = db.get_sale_category(category_id) if category_id else None
        form_id = int(cat.get("form_id", 1)) if cat else 1
        db.delete_sale_product(int(product_id))
        meta.update({"view": "category_products", "form_id": form_id, "category_id": category_id})
        return ("ลบรายการสินค้าเรียบร้อยแล้ว", "ok", meta)
    ...
    if action == "shop_product_dispatch":
        ...
        prod = db.get_sale_product(int(product_id))
        if not prod:
            return ("ไม่พบรายการสินค้า", "warn", meta)
        channel_id = channel_id or str(prod.get("channel_id", "")).strip()
        if not channel_id:
            return ("ไม่มี channel_id สำหรับส่งการ์ด", "warn", meta)
        db.enqueue_sale_card(int(product_id), channel_id)
        cat = db.get_sale_category(int(prod.get("category_id", 0))) if prod.get("category_id") else None
        form_id = int(cat.get("form_id", 1)) if cat else 1
        meta.update({"view": "category_products", "form_id": form_id, "category_id": int(prod.get("category_id", 0))})
        return ("เพิ่มคิวส่งการ์ดเรียบร้อยแล้ว", "ok", meta)

    return ("ไม่พบ action ที่ต้องการ", "warn", meta)
...
def application(environ, start_response):
    ...
    view = (query.get("view", ["dashboard"])[0] or "dashboard").strip()
    q = (query.get("q", [""])[0] or "").strip()
    form_id = _safe_int(query.get("form_id", ["0"])[0])
    category_id = _safe_int(query.get("category_id", ["0"])[0])
    edit_category_id = _safe_int(query.get("edit_category_id", ["0"])[0])
    edit_product_id = _safe_int(query.get("edit_product_id", ["0"])[0])
    ...
    if path in {"/save", f"{panel_path}/save", f"{panel_path_alt}save"} and method == "POST":
        ...
        if action.startswith("shop_"):
            note, level, meta = _handle_shop_action(form)
            body = _render_page(
                _env(),
                note=note,
                level=level,
                view=str(meta.get("view", form.get("view", "dashboard"))),
                q=q,
                form_id=_safe_int(str(meta.get("form_id", form_id))),
                category_id=_safe_int(str(meta.get("category_id", category_id))),
                edit_category_id=_safe_int(str(meta.get("edit_category_id", edit_category_id))),
                edit_product_id=_safe_int(str(meta.get("edit_product_id", edit_product_id))),
            )
            start_response("200 OK", [("Content-Type", "text/html; charset=utf-8")])
            return [body.encode("utf-8")]
